...
Phase 03은 Phase 01 factory·보안 설정과 Phase 02 인증·사용자 기능 위에 상품 CRUD,
안전한 이미지 제공과 상품 검색을 구현한다. 전체·1대1 채팅, 신고, 송금과 관리자 UI route는
공개하지 않는다.
...
| `tests/` | foundation, 인증, 사용자, 상품, 이미지, 검색, migration 회귀 테스트 |
...
Phase 01 `09357cac1cb7`과 Phase 02 `57c21fbc6f83`은 수정하지 않는다. Phase 03
`c3d57de11bfa`는 두 번째 revision을 `down_revision`으로 사용한다. SQLite batch operation으로
가격 CHECK를 bounded named CHECK로 교체하고 nullable image filename UNIQUE와 세 named
index를 추가한다. downgrade는 index·UNIQUE·bounded CHECK를 제거하고 기존 positive CHECK를
복구한다. 검증 범위는 빈 DB head upgrade/current/check, Phase 02 downgrade, head
재-upgrade와 재 check다.

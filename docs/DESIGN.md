...
Phase 06은 Phase 01~05 factory·인증·사용자·상품·이미지·검색·채팅·신고·관리자·감사
기능 위에 과제용 가상 포인트 송금과 최종 통합을 구현한다. 실제 은행·카드·결제·환전·충전·
출금·환불 자산이나 외부 결제 API는 범위에 포함하지 않는다.
...
| `app/wallet/policy.py` | 송금 amount·recipient username·raw token 정책과 derived key 생성 |
| `app/wallet/forms.py` | transfer current-password·recipient·amount·hidden token, history query 검증 |
| `app/wallet/routes.py` | Wallet HTTP 인증·CSRF·rate limit·303/409/404·private cache 흐름 |
| `app/wallet/services.py` | idempotency 예약, 조건부 debit, credit, audit transaction과 history/detail query |
| `app/wallet/views.py` | frozen slots Wallet summary·history page·detail DTO |
| `app/cli.py` | hidden prompt 기반 CLI-only admin 원자 생성 |
| `app/models/` | User·Wallet·Product·Transfer 등 모델과 DB 제약 |
| `tests/` | 기존 회귀와 HTTP·DB·Socket·transaction·idempotency·concurrency 테스트 |
...
02 가입과 Phase 05 CLI admin 생성은 명시적으로 100000을 설정한다. Phase 06 송금도 balance
총합을 바꾸지 않는다. 이 포인트는 과제용 가상 정수이며 실제 금융 자산, 현금 또는 결제
수단이 아니다.
...
| GET | `/wallet` | 필요 | 해당 없음 | IP 60/minute |
| GET | `/wallet/transfer` | 필요 | 해당 없음 | IP 60/minute |
| POST | `/wallet/transfer` | 필요 | 필요+current password | 사용자 3/minute·10/hour |
| GET | `/wallet/transfers/<uuid>` | 필요·sender/recipient | 해당 없음 | IP 60/minute |
...
영구 cloud 배포, 실제 결제와 다중-process 확장은 현재 과제 범위 밖이다.
...
password 변경, Phase 05 자동 제재와 관리자 status 전이에서 즉시 사용한다.
...
않았으며 Phase 06 최종 통합 뒤에도 다중-process 운영 잔여 위험으로 유지한다.
...
없었으며 Phase 06의 실제 원장을 같은 idempotency key·내부 user ID 제외 projection으로
조회한다. 관리자 Transfer route는 계속 GET-only다.
...
## Phase 06 Wallet 입력과 DTO 경계

`TransferForm`은 recipient username, amount, strip하지 않는 current password와 hidden
idempotency token만 받는다. sender ID는 항상 현재 authenticated user에서 가져오고 recipient는
strip한 username으로 active User와 Wallet을 서버에서 다시 조회한다. `sender_id`,
`recipient_id`, `balance`, `idempotency_key` 같은 extra field는 service 입력이 아니며
무시한다.

송금액은 bool이 아닌 정수 1~1,000,000,000만 허용한다. current password는 최대 128자이며
검증 뒤 저장·반사·로그하지 않는다. dormant sender는 version-aware loader에서 차단되고
dormant recipient와 Wallet 없는 사용자는 같은 일반 unavailable 결과를 사용한다. 자기
송금도 form 값이 아니라 조회된 두 User ID를 비교해 거부한다.

기본 설정은 `TRANSFER_MIN_AMOUNT=1`, `TRANSFER_MAX_AMOUNT=1_000_000_000`,
`TRANSFER_HISTORY_PER_PAGE=20`, `TRANSFER_PAGE_MAX=1000`,
`TRANSFER_IDEMPOTENCY_TOKEN_BYTES=32`다. service 결과는 `CREATED`, `IDEMPOTENT`,
`IDEMPOTENCY_CONFLICT`, `SELF_TRANSFER`, `RECIPIENT_UNAVAILABLE`,
`CURRENT_PASSWORD_INVALID`, `INSUFFICIENT_FUNDS`, `DATABASE_ERROR`처럼 route가 안전한
HTTP 결과로 변환할 수 있는 유한 상태로 제한한다.

template에는 전체 User, Wallet, Transfer ORM 객체를 전달하지 않는다.

- `WalletSummaryView`: username, balance
- `TransferHistoryView`: transfer id, sent/received 방향, counterpart username, amount,
  created time
- `TransferHistoryPage`: items와 fixed 20 pagination metadata
- `TransferDetailView`: transfer id, sender/recipient username, 현재 사용자 방향, amount,
  created time

DTO와 SELECT projection에는 sender/recipient ID, idempotency key, raw token, password hash,
auth version, role, session 값과 Wallet ORM 객체가 없다.

## Idempotency token과 sender namespace

`GET /wallet/transfer`마다 `secrets.token_urlsafe(32)`로 일반적으로 padding 없는 정확한 43자
URL-safe token을 새로 만들고 hidden input으로만 전달한다. service는 type, 정확한 길이와
`A-Z`, `a-z`, `0-9`, `_`, `-` 문자만 다시 검사한다. raw token은 화면 텍스트, flash, 응답
detail, 일반 로그, AuditLog와 DB에 저장하지 않는다.

저장 key는 다음과 같다.

```text
sha256(str(sender_user_id) + ":" + raw_token).hexdigest()
```

결과는 sender namespace에 속하는 64자리 소문자 hex다. 같은 key의 기존 Transfer가 sender,
recipient와 amount까지 같으면 기존 transfer ID를 반환하고 추가 balance·ledger·audit 변경을
하지 않는다. recipient 또는 amount가 다르면 idempotency conflict로 처리하고 상태를
변경하지 않는다. key 자체도 사용자·관리자 projection이나 audit에 포함하지 않는다.

## 조건부 debit과 원자 transaction

신규 송금의 순서는 다음과 같다.

```text
active sender + sender Wallet + current password
  -> active recipient username + recipient Wallet + self/amount/token 검증
  -> derived key 기존 Transfer 확인
  -> Transfer add + flush로 UNIQUE key 예약
  -> UPDATE wallets
       SET balance = balance - amount
       WHERE user_id = sender_id AND balance >= amount
  -> debit rowcount == 1 확인
  -> recipient balance = balance + amount 조건부 UPDATE
  -> credit rowcount == 1 확인
  -> AuditLog(action=transfer.created, target=transfer, details={"amount": amount})
  -> one commit
```

Python에서 balance를 읽어 차감하지 않는다. 조건부 SQL UPDATE와 rowcount가 동시 송금의
overspend를 막는다. debit, credit, Transfer와 audit는 같은 transaction이므로 잔액 부족,
Wallet 누락, credit 실패, audit 실패와 commit 실패는 전부 rollback한다. audit details에는
amount만 허용하고 recipient ID/username, raw/derived token, balance와 password는 넣지 않는다.

`IntegrityError`는 먼저 rollback한 뒤 같은 derived key Transfer를 새 session 상태에서
재조회한다. 같은 payload면 idempotent 결과, 다른 payload면 conflict, 그 외에는 일반
database error다. 성공과 idempotent 성공만 detail로 303 redirect하며 conflict는 409,
예상 가능한 나머지 실패는 SQL·constraint·내부 ID를 숨긴 일반 메시지로 처리한다.

## 동시성, SQLite와 history query

file-based SQLite의 독립 session/thread 테스트는 balance 100에서 서로 다른 token 80+80을
동시에 보내도 둘 다 성공하지 않고, 성공한 Transfer 수와 debit 횟수가 같으며 모든 Wallet
총합이 유지되는지 검사한다. 같은 token·payload의 동시 요청은 Transfer와 debit/credit를
정확히 한 번만 만들고 나머지는 idempotent 결과여야 한다.

SQLite 연결은 기존 `foreign_keys=ON`을 유지하고 busy timeout 5000ms 또는 동등한 connection
timeout을 적용한다. WAL mode는 강제하지 않는다. 이는 짧은 과제용 transaction의 경쟁을
완화하지만 SQLite의 높은 write concurrency 한계를 제거하지 않는다. 다중 process·고부하
배포는 외부 shared limiter와 더 강한 transaction 격리를 제공하는 DB가 필요하다.

history query는 현재 user가 sender 또는 recipient인 row만 대상으로 `all`, `sent`,
`received`와 `newest`, `oldest` allowlist를 적용한다. sender/recipient User alias에서
counterpart username을 필요한 column만 SELECT하고 created_at과 id를 안정 정렬한 뒤 fixed
20 LIMIT/OFFSET을 SQL에서 수행한다. page는 1~1000이며 client `per_page`는 읽지 않는다.
detail도 sender/recipient participant 조건을 query에 포함해 제3자와 missing을 같은 404로
처리한다.

## 여섯 번째 migration과 CI

기존 다섯 revision을 수정하지 않고 `e5b7a2c9d4f1` 다음 여섯 번째 revision 하나만 추가한다.
기존 `ck_transfers_amount_positive`를 `ck_transfers_amount_range`로 교체하고
`amount BETWEEN 1 AND 1000000000`을 강제한다. `ck_transfers_idempotency_key_format`은
정확한 64자리 소문자 hex를 강제한다. `ix_transfers_sender_created`와
`ix_transfers_recipient_created`는 participant history의 `(user_id, created_at, id)` query를
지원한다. 기존 idempotency UNIQUE, sender/recipient FK와 distinct-user CHECK는 유지한다.
downgrade는 새 index와 CHECK를 제거하고 positive amount CHECK를 복구한다.

GitHub Actions는 push와 pull request에서 Python 3.12, pinned development dependency,
pytest와 app coverage 90%, Ruff lint/format, Bandit, runtime/development pip-audit, pip check,
compileall, 빈 SQLite DB upgrade와 `flask db check`를 수행한다. workflow가 생성한 무작위
Secret Key와 임시 DB만 사용하고 `.env`나 repository secret을 요구하지 않는다. 최종 PASS
수치와 coverage는 실제 전체 검증이 끝난 뒤 기록한다.

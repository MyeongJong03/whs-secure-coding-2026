...
| ID | 독립 요구사항 | 구현 단계 |
|---|---|---|
| FR-501 | 인증 사용자는 다른 사용자를 신고할 수 있다. | Phase 05 예정 |
| FR-502 | 인증 사용자는 다른 사용자의 상품을 신고할 수 있다. | Phase 05 예정 |
| FR-503 | 사용자·상품 신고에는 정책에 맞는 신고 사유가 필요하다. | DB 기반 Phase 01, 서비스 Phase 05 예정 |
| FR-504 | 자기 자신과 자신이 소유한 상품은 신고할 수 없다. | Phase 05 예정 |
| FR-505 | 동일 신고자가 동일 대상을 중복 신고할 수 없다. | DB 기반 Phase 01, 서비스 Phase 05 예정 |
| FR-506 | 서로 다른 신고자 3명이 같은 상품을 유효하게 신고하면 상품을 hidden 처리한다. | Phase 05 예정 |
| FR-507 | 서로 다른 신고자 3명이 같은 사용자를 유효하게 신고하면 사용자를 dormant 처리한다. | Phase 05 예정 |
| FR-508 | 관리자는 신고를 검토하고 대상 상태를 복구할 수 있다. | Phase 05 예정 |

## 송금 요구사항

| ID | 독립 요구사항 | 구현 단계 |
|---|---|---|
| FR-601 | 회원가입 시 사용자 Wallet에 가상 포인트 100000을 지급한다. | Phase 01 DB + Phase 02 가입 transaction 구현 |
| FR-602 | 포인트가 실제 금융 자산이 아닌 과제용 가상 포인트임을 UI와 문서에 명시한다. | Phase 02 마이페이지·문서 구현, Phase 06 송금 UI 예정 |
| FR-603 | 인증 사용자는 다른 사용자에게 가상 포인트를 송금할 수 있다. | Phase 06 예정 |
| FR-604 | 자기 자신에게 송금할 수 없다. | DB 기반 Phase 01, 서비스 Phase 06 예정 |
| FR-605 | 송금액은 양수 정수만 허용한다. | DB 기반 Phase 01, 서비스 Phase 06 예정 |
| FR-606 | 현재 잔액을 초과하는 송금을 거부한다. | 잔액 DB 제약 Phase 01, 서비스 Phase 06 예정 |
| FR-607 | 검증, 차감, 증가와 원장 생성을 단일 DB 트랜잭션으로 수행한다. | Phase 06 예정 |
| FR-608 | 송금 요청마다 idempotency key를 사용하고 재요청은 같은 결과로 처리한다. | DB unique Phase 01, 서비스 Phase 06 예정 |
| FR-609 | 완료된 송금은 수정·삭제하지 않는 의미의 원장 레코드로 보존한다. | 모델 기반 Phase 01, 서비스 정책 Phase 06 예정 |

## 관리자 요구사항

| ID | 독립 요구사항 | 구현 단계 |
|---|---|---|
| FR-701 | 관리자는 사용자 상태와 허용된 사용자 관리 작업을 수행할 수 있다. | Phase 05 예정 |
| FR-702 | 관리자는 상품 상태와 허용된 상품 관리 작업을 수행할 수 있다. | Phase 05 예정 |
| FR-703 | 관리자는 정책상 필요한 채팅 메시지를 검토할 수 있다. | Phase 05 예정 |
| FR-704 | 관리자는 신고를 검토하고 처리할 수 있다. | Phase 05 예정 |
| FR-705 | 관리자는 송금 내역을 읽기 전용으로 조회할 수 있다. | Phase 05 예정 |
| FR-706 | 모든 관리자 조치를 감사 로그에 기록한다. | 모델 기반 Phase 01, 서비스 Phase 05 예정 |
| FR-707 | 일반 사용자의 모든 관리자 URL 접근을 서버에서 거부한다. | Phase 05 예정 |

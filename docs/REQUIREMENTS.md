...
Phase 01은 안전한 실행 기반을 확정했고 Phase 02는 인증·사용자 관리 수직 기능을 구현한다.
...
| ID | 독립 요구사항 | 구현 단계 |
|---|---|---|
| FR-201 | 인증 사용자는 상품을 등록할 수 있다. | Phase 03 예정 |
| FR-202 | 상품은 상품명, 설명, 가격, 이미지와 판매자 정보를 제공한다. | 모델 기반 Phase 01, 서비스 Phase 03 예정 |
| FR-203 | 비로그인 사용자도 공개 상품 목록을 조회할 수 있다. | Phase 03 예정 |
| FR-204 | 비로그인 사용자도 공개 상품 상세를 조회할 수 있다. | Phase 03 예정 |
| FR-205 | 인증 사용자는 본인이 등록한 상품 목록을 조회할 수 있다. | Phase 03 예정 |
| FR-206 | 상품 소유자는 본인 상품을 수정할 수 있다. | Phase 03 예정 |
| FR-207 | 상품 소유자는 본인 상품을 삭제 상태로 변경할 수 있다. | Phase 03 예정 |
| FR-208 | 상품 소유자는 본인 상품의 판매 상태를 관리할 수 있다. | Phase 03 예정 |
| FR-209 | 다른 사용자의 상품 수정 요청을 거부한다. | Phase 03 예정 |
| FR-210 | 다른 사용자의 상품 삭제 요청을 거부한다. | Phase 03 예정 |
| FR-211 | 상품 정보와 상태는 DB에서 관리한다. | DB 기반 Phase 01, 서비스 Phase 03 예정 |
| FR-212 | 상품 이미지는 JPEG, PNG, WebP만 안전하게 처리한다. | Phase 03 예정 |

## 검색 요구사항

| ID | 독립 요구사항 | 구현 단계 |
|---|---|---|
| FR-301 | 상품명과 상품 설명을 대상으로 검색한다. | Phase 03 예정 |
| FR-302 | 허용된 상품 상태로 검색 결과를 필터링한다. | Phase 03 예정 |
| FR-303 | 최소·최대 가격 범위로 검색 결과를 필터링한다. | Phase 03 예정 |
| FR-304 | 서버 allowlist에 포함된 정렬 기준과 방향만 허용한다. | Phase 03 예정 |
| FR-305 | 검색 결과를 페이지네이션하고 hidden·deleted 상품은 제외한다. | Phase 03 예정 |
...
| FR-201~212, FR-301~305 | DESIGN의 후속 단계 | T-DB-04는 가격 DB 제약만 검증 | Phase 03 예정 |
...
| SR-001~016 | DESIGN·THREAT_MODEL·SECURITY_FINDINGS | TEST_MATRIX의 자동·도구 검증 | 인증·사용자 범위 구현, 후속 범위 예정 |
| NFR-001~007 | DESIGN·README | T-MIG-*, T-TOOL-* | Phase 02 범위 구현·지속 검증 |

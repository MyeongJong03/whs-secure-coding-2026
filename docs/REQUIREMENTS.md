...
- Phase 05: 신고, 자동 제재, 관리자 기능 — 현재 구현
- Phase 06: 가상 포인트 송금, 최종 통합 및 보안 강화

Phase 05에는 실제 가상 포인트 송금 mutation이 포함되지 않는다. 예정 요구사항은 후속 구현
범위를 정의할 뿐 현재 전체 플랫폼 구현 완료를 의미하지 않는다.
...
| SR-001~020 | DESIGN·THREAT_MODEL·SECURITY_FINDINGS | TEST_MATRIX의 자동·도구 검증 | Phase 05 범위까지 구현, 송금 후속 범위 예정 |
| NFR-001~007 | DESIGN·README | T-MIG-*, T-CHAT-STATIC-*, T-P05-*, T-TOOL-* | Phase 05 범위 구현·지속 검증 |

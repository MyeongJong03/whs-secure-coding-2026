...
## Phase 05 migration·신고·자동 제재

| Test ID | 입력/행동 | 기대 결과 | 상태 |
|---|---|---|---|
| T-P05-MIG-01 | revision source와 파일 수 | `a91f4c8d2e70` 뒤 다섯 번째 revision, 정확히 5개 | PASS |
| T-P05-MIG-02 | phase-04-chat의 기존 네 migration | byte 단위 동일 | PASS |
| T-P05-MIG-03 | 빈 DB upgrade/check→Phase 04 downgrade→upgrade/check | 새 metadata만 가역, drift 없음 | PASS |
| T-P05-MIG-04 | product previous status DB 입력 | NULL/active/sold 허용, 다른 값 거부 | PASS |
| T-P05-MIG-05 | report pending/reviewed 상태 조합 | pending reviewer NULL, 결정 상태 reviewer/timestamp 필수 | PASS |
| T-P05-MIG-06 | AuditLog 빈 action/target type | named CHECK로 거부 | PASS |
| T-P05-REPORT-01 | 사용자·상품 정상 신고와 spoof field | current reporter와 URL target/type, pending 저장 | PASS |
| T-P05-REPORT-02 | 9/501/byte/control/NUL/DEL/non-string | form/policy 거부, 저장 없음 | PASS |
| T-P05-REPORT-03 | CRLF/CR·decomposed Unicode·공백 | LF/NFC/strip 정규 저장 | PASS |
| T-P05-REPORT-04 | 자기 사용자·자기 상품·없는/dormant/hidden | 404 또는 일반 unavailable, 저장 없음 | PASS |
| T-P05-REPORT-05 | 동일 reporter-target 재신고·직접 UNIQUE race | 한 row만 유지, 일반 duplicate 처리 | PASS |
| T-P05-REPORT-06 | 두 report endpoint 교차 11 POST | shared user quota의 11번째 429 | PASS |
| T-P05-REPORT-07 | 내 신고 목록 | 본인만 fixed 20 projection DTO, 내부 target ID/version 없음 | PASS |
| T-P05-AUTO-01 | active/sold 상품의 서로 다른 1·2·3명 | 3번째 hidden, previous status와 system audit | PASS |
| T-P05-AUTO-02 | rejected 신고 뒤 추가 신고 | rejected 제외한 유효 3명에서만 hidden | PASS |
| T-P05-AUTO-03 | 일반 사용자 서로 다른 3명 | dormant, version +1, system audit, registry/socket disconnect | PASS |
| T-P05-AUTO-04 | admin 대상 서로 다른 3명 | pending 저장, active/version 유지 | PASS |
| T-P05-AUTO-05 | audit/DB failure | report·target status·version 전체 rollback | PASS |

## Phase 05 admin·audit·CLI

| Test ID | 입력/행동 | 기대 결과 | 상태 |
|---|---|---|---|
| T-P05-CLI-01 | create-admin 정상 hidden prompt | admin/active/version 1+scrypt+Wallet 100000+system audit | PASS |
| T-P05-CLI-02 | mismatch·11/129자·잘못된/중복 username | 전체 거부·rollback, 평문/hash 출력 없음 | PASS |
| T-P05-CLI-03 | 강제 commit failure·help | User/Wallet/Audit 없음, plaintext password option 없음 | PASS |
| T-P05-RBAC-01 | anonymous/user/dormant/active admin 전체 GET | 차단/403 또는 login redirect/200, 모두 no-store | PASS |
| T-P05-RBAC-02 | mutation CSRF 누락·wrong password | 400 또는 generic reauth 실패, 상태 불변·비밀번호 미반사 | PASS |
| T-P05-USER-01 | active→dormant→active | 각 실제 전이 version 증가, audit commit 뒤 disconnect | PASS |
| T-P05-USER-02 | 자기/다른 admin·role spoof | self/last 보호, 다른 admin 조건부 전이, role 불변 | PASS |
| T-P05-USER-03 | old cookie/CSRF 뒤 restore와 새 login | 과거 인증 mutation 불가, 새 login만 허용 | PASS |
| T-P05-PRODUCT-01 | active/sold hide·hidden restore | previous status 저장/복구/NULL 초기화 | PASS |
| T-P05-PRODUCT-02 | delete·deleted hide/restore·mass assignment | row/image 유지 deleted, 복구 거부, seller/status spoof 무시 | PASS |
...
| T-P05-ADMIN-DTO-01 | 여섯 관리자 목록 context | fixed 50 전용 DTO, 전체 ORM·민감 field 없음 | PASS |
...
| 명령/범위 | 결과 |
|---|---|
| `.venv/bin/python -m pytest` | PASS, 518 tests |
| `.venv/bin/python -m pytest --cov=app --cov-report=term-missing` | PASS, 518 tests, app 96% |
